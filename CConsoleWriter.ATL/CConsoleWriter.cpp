...
{
    ATL::CString cstrLine("");
    if (action)
    {

        cstrLine.SetString(action, cstrLine.StringLength(action));
        if (prep && prep != L"")
        {
            cstrLine.Append(L": ");
    ...
    if (this->m_OutputLevel)
        this->WriteStringToConsole(L"%s", cstrLine.GetBuffer());
    cstrLine.ReleaseBuffer();
    return 0;
}
    ...
    if (buffer)
    {
        ...
        if(StringCchPrintfW(buffer, 0x3Du, L"[%58s]", L"")>=0 && StringCchPrintfW(lpszPercent, 9, L"%2.1f%%", percent * 100.0) >= 0)
        {
            if (wcslen(lpszPercent) < wcslen(buffer))
            {
                ...
                if (message)
                {
                    cstrUserMsg.Format(L"%-40s",message);
                    cstrUserMsg = cstrUserMsg.Trim();
                }
...
HRESULT CConsoleWriter::TerminateProgressBar(const wchar_t* message)
{
    ...
    if (this->m_OutputLevel)
        CConsoleWriter::WriteStringToConsole(L"%s", cstrLine.GetBuffer());
    cstrLine.ReleaseBuffer();
    return 0;
}
